...
#复制阿里云yum源配置文件和kubernetes.conf内核参数文件并安装依赖包
if [[ ! -e /usr/local/bin/cfssl || ! -e /usr/local/bin/cfssljson ]];then
    wget http://120.197.59.147:38080/cfssl -O /usr/local/bin/cfssl 
    #
    wget http://120.197.59.147:38080/cfssljson -O /usr/local/bin/cfssljson
    #
fi
chmod a+x /usr/local/bin/*
mkdir -p /etc/kubernetes/pki/CA &> /dev/null
...
if [[ ! -e /tmp/etcd-v3.3.10-linux-amd64.tar.gz ]];then
    wget http://120.197.59.147:38080/etcd-v3.3.10-linux-amd64.tar.gz -O /tmp/etcd-v3.3.10-linux-amd64.tar.gz
    tar xvf /tmp/etcd-v3.3.10-linux-amd64.tar.gz -C /tmp
fi
...
setKubectl(){
    if [[ ! $(which kube-apiserver) ]];then
        wget http://120.197.59.147:38080/$k8sVersion/kubernetes-server-linux-amd64.tar.gz -O /opt/kubernetes-server-linux-amd64.tar.gz && tar xvf /opt/kubernetes-server-linux-amd64.tar.gz  -C /opt/&& cd /opt/kubernetes/server/bin && rm -rf *.tar *.docker_tag 
        #如果上述链接失效，请使用如下链接
        #wget http://dl.k8s.io/$k8sVersion/kubernetes-server-linux-amd64.tar.gz -O /opt/kubernetes-server-linux-amd64.tar.gz && tar xvf /opt/kubernetes-server-linux-amd64.tar.gz  && cd /opt/kubernetes/server/bin && rm -rf *.tar *.docker_tag && mv * /usr/local/bin/
        for i in ${nodeCount[*]};do
            scp /opt/kubernetes/server/bin/* $i:/usr/local/bin/
...
if [[ ! $(which flanneld) ]];then
   wget http://120.197.59.147:38080/flannel-v0.10.0-linux-amd64.tar.gz -O /opt/flannel-v0.10.0-linux-amd64.tar.gz 
   tar xvf /opt/flannel-v0.10.0-linux-amd64.tar.gz -C /opt/
   cp /opt/{flanneld,mk-docker-opts.sh} /usr/local/bin/
fi
...
sleep 10
for i in $(kubectl get csr | awk 'NR>1{print $1}' );do kubectl certificate approve $i ;done
wget http://120.197.59.147:38080/pause-amd64-3.0.tar.gz -O /tmp/pause-amd64-3.0.tar.gz
for i in ${NodeIP[*]};do
    scp /tmp/pause-amd64-3.0.tar.gz $i:/tmp
    ssh $i "docker image load -i /tmp/pause-amd64-3.0.tar.gz"
done
...
deployIngressController(){
	echo -e "\033[32m 正在部署nginx-ingress-controller.. \033[0m"
    if [ ! -e /tmp/nginx-ingress-controller-0.27.1.tar.gz ];then
        wget http://120.197.59.147:38080/nginx-ingress-controller-0.27.1.tar.gz -O /tmp/nginx-ingress-controller-0.27.1.tar.gz
    fi
    wget http://120.197.59.147:38080/nginx-ingress-controller-mandatory.yaml -O /tmp/nginx-ingress-controller-mandatory.yaml
    wget http://120.197.59.147:38080/nginx-ingress-controller-service.yaml -O /tmp/nginx-ingress-controller-service.yaml
    for i in ${NodeIP[*]};do
        scp /tmp/nginx-ingress-controller-0.27.1.tar.gz /tmp/nginx-ingress-controller-mandatory.yaml $i:/tmp/
        ssh $i exec  "docker image load -i /tmp/nginx-ingress-controller-0.27.1.tar.gz"
    done
...
deployCoreDNS(){
   ...
   if [ ! -e /tmp/coredns-deployment-1.8.0.tar.gz ];then
        wget http://120.197.59.147:38080/coredns-deployment-1.8.0.tar.gz -O /tmp/coredns-deployment-1.8.0.tar.gz
        tar xvf /tmp/coredns-deployment-1.8.0.tar.gz -C /tmp
   fi
   if [ ! -e /tmp/coredns-image-1.8.0.tar.gz ];then
        wget http://120.197.59.147:38080/coredns-image-1.8.0.tar.gz -O /tmp/coredns-image-1.8.0.tar.gz
   fi
